...
# Add noise to training data, add to original training data to form augmented training set
...
# Transform the testing_data data
X_test_final = preprocessor.transform(X_test)

# Add Gaussian noise to the numerical features of the training set
sigma = 0.15
X_train_noisy = X_train_final.copy()
X_train_noisy[:, :len(numeric_cols)] = X_train_final[:, :len(numeric_cols)] + np.random.normal(0, sigma, X_train_final[:, :len(numeric_cols)].shape)

# Concatenate original data and noisy data
X_train_augmented = np.vstack([X_train_final, X_train_noisy])

# Concatenate original labels and labels for noisy data (which are the same as the original labels)
y_train_augmented = np.concatenate([y_train, y_train])
# Convert to DataFrame
X_test_final_df = pd.DataFrame(X_test_final)
X_train_final_df = pd.DataFrame(X_train_augmented)
# Plot histograms
...
X_train_final_df['Danceability'] = y_train_augmented
X_train_final_df.to_csv('../training_data/train_augmented.csv')
X_test_final_df.to_csv('../testing_data/test_augmented.csv')
