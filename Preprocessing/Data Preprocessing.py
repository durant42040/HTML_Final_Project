...
sigma = 0.1
X_train_final[:, :len(numeric_cols)] = X_train_final[:, :len(numeric_cols)] + np.random.normal(0, sigma,
                                                                                            X_train_final[:, :len(
                                                                                                   numeric_cols)].shape)

# Convert to DataFrame
X_train_final_df = pd.read_csv("../training_data/train_encoded.csv")
X_test_final_df = pd.read_csv("../testing_data/test_encoded.csv")
# Plot histograms
